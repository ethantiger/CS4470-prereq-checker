...
import {
    ...
    PrereqGroup,
    CourseData,
    PrereqGroupWithCredits
} from '@/types';

// AUTHOR: Tyler Larson
// This component is for adding a new course to the database. It includes fields for
// course code, name, department, weight, antirequisites, and prerequisites.
...
export default function AddCourse({
    ...
    const [isAntireqInputActive, setIsAntireqInputActive] = useState(false);
    ...
    const renderPrereqGroup = (
        group: PrereqGroup | PrereqGroupWithCredits,
        depth: number = 0,
        path: number[] = []
    ): JSX.Element => {
    ...
    const addAntireq = () => {
        const value = antireqInput.trim().toUpperCase();
        const currentCode = courseCode.trim().toUpperCase();

        if (!value) return;

        if (value === currentCode) {
            setAntireqInput('');
            setIsAntireqInputActive(false);
            return;
        }

        if (antireqs.some(a => a.toUpperCase() === value)) {
            setAntireqInput('');
            setIsAntireqInputActive(false);
            return;
        }
    ...
    const filteredAntireqOptions = Object.keys(courses)
        .filter(code => code.toUpperCase() !== courseCode.trim().toUpperCase())
        .filter(code => !antireqs.some(a => a.toUpperCase() === code.toUpperCase()))
                    ...
                    <div style={{ display: 'flex', gap: '0.75em', alignItems: 'flex-start' }}>
                        <div style={{ position: 'relative' }}>
                            <input
                                ...
                                value={antireqInput}
                                onChange={(e) => {
                                    setAntireqInput(e.target.value);
                                    setIsAntireqInputActive(true);
                                }}
                                onFocus={() => setIsAntireqInputActive(true)}
                                onBlur={() => setTimeout(() => setIsAntireqInputActive(false), 200)}
                                style={{
                                    padding: '0.6em 0.75em',
                                    width: '300px',
                                    fontSize: '0.95em',
                            ...
                            {isAntireqInputActive &&
                                antireqInput.trim() !== '' &&
                                filteredAntireqOptions.length > 0 && (
                                    <div
                                        style={{
                                            position: 'absolute',
                                            top: '100%',
